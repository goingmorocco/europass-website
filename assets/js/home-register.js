...
// leave the homepage to create an account. Runs on both the English and
// Arabic homepages, so every user-facing string branches on language.
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Portal dashboards only exist at the site root, never under /ar/ — this
  // same script runs on both homepages, so redirect targets need this
  // prefix when called from inside /ar/, or they'd resolve to a
  // non-existent /ar/student-dashboard.html.
  const isArabic = /\/ar\//.test(window.location.pathname);
  const portalPrefix = isArabic ? '../' : '';
